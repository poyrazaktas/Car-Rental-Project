...
export class BankService {
  ...
  checkCardInfo(cardInformation: CardInformation): Observable<ResponseModel> {
    ...
    return this.httpClient.post<ResponseModel>(this.apiUrl + extension, cardInformation).pipe(catchError(this.errorHandler));
  }

  pay(price: number): Observable<ResponseModel> {
    const extension = 'cardinformations/pay?price=';
    return this.httpClient.get<ResponseModel>(this.apiUrl + extension + price).pipe(catchError(this.errorHandler));
  }
